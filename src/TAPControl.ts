...
export default class TAPControl {
    ...
    private constructor(findbyip:string) {
        ...
        for (const device of allDevicesInfo) {
            if ( device.description.toLocaleLowerCase().indexOf("tap-windows adapter v9") !== -1) {
                //console.log(device)
                if (device.gatewayIpAddress.toLocaleLowerCase().indexOf(findbyip.toLocaleLowerCase()) !== -1){
        ...
        if (this.deviceInfo === null) {
            console.log("Openvpn adapter not found for ip "+findbyip+", using fallback instead!!!!!");
            for (const device of allDevicesInfo) {
                if ( device.description.toLocaleLowerCase().indexOf("tap-windows adapter v9") !== -1) {
                       this.deviceInfo = device;
                }
        ...
        if (this.deviceInfo === null) {
            throw new NotFoundError("Openvpn adapter not found for ip "+findbyip);
        }
    }

    public static init(findbyip:string): TAPControl {
        if (this.tapControlObject === null) {
            this.tapControlObject = new TAPControl(findbyip);
        }
